...
class Control(object):
    ...
    def gen(self, inst):
        ...
        # (added) Control signal to select the next PC 
        ...
        # (added) Check for data hazard
        data_hazard         = (EX.reg_rd == Pipe.FD.rs1 and EX.reg_rd != 0 and EX.reg_c_rf_wen and rs1_oen) or    \
                              (EX.reg_rd == Pipe.FD.rs2 and EX.reg_rd != 0 and EX.reg_c_rf_wen and rs2_oen) or    \
                              (MW.reg_rd == Pipe.FD.rs1 and MW.reg_rd != 0 and Pipe.MW.c_rf_wen and rs1_oen) or   \
                              (MW.reg_rd == Pipe.FD.rs2 and MW.reg_rd != 0 and Pipe.MW.c_rf_wen and rs2_oen)                  
   
        # (added) Check for mispredicted branch/jump
        EX_brjmp            = self.pc_sel != PC_4

        # (added)
        self.FD_stall       = not EX_brjmp and data_hazard
        self.EX_bubble      = data_hazard or EX_brjmp
...
class FD(Pipe):
    ...
    def compute(self):
        ...
        # (modified & added) self.op1_data -> rf_rs1_data
        ...
        # (added) Determine ALU operand 1: PC or R[rs1]
...
class EX(Pipe):
    ...
    def compute(self):
        ...
        # (modified) The second input to ALU should be put into self.alu2_data for correct log msg.
        ...
        # (added) Adjust the output for jalr instruction (forwarded to FD)
        self.jump_reg_target    = self.alu_out & WORD(0xfffffffe)

        # (added) Calculate the branch/jump target address using an adder (forwarded to FD)
        self.brjmp_target       = Pipe.cpu.adder_brtarget.op(self.pc, self.op2_data)

        # (added) For jal and jalr instructions, pc+4 should be written to the rd
...
class MW(Pipe):
    ...
    def compute(self):
        ...
        # (added) Access data memory (dmem) if needed
        mem_data, status = Pipe.cpu.dmem.access(self.c_dmem_en, self.alu_out, self.rs2_data, self.c_dmem_rw)
        
        # (added) For load instruction, we need to store the value read from dmem
        self.wbdata         = mem_data          if self.c_wb_sel == WB_MEM  else \
                              self.alu_out  

        # (added) Handle exception during dmem access
        if not status:
    ...
    def update(self):
        ...
        # (modified) self.alu_out -> self.wbdata
        if self.c_rf_wen:
